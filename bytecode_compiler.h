...
/// 将 GDScript 源码 token 化并打包为可执行的字节码，供脚本在运行时以二进制形式保存或加载。[br][br]
/// 该类只负责「源码 → 字节码」的编译以及「字节码 → 压缩字节码」的打包，[br]
...
public:
	...
	/// 将 GDScript 源码字符串 token 化并生成字节码。[br][br]
	/// [param p_source_code] 待编译的 GDScript 源码文本，空文本无法编译[br]
	/// [param p_compression] 是否压缩生成的字节码，默认不压缩[br]
	...
	/// 从脚本对象读取其源码并生成字节码。[br][br]
	/// [param p_script] 待编译的脚本，仅接受具有源码的 GDScript，其它脚本类型会被拒绝[br]
	/// [param p_compression] 是否压缩生成的字节码，默认不压缩[br]
	...
	/// 校验字节码头并对其负载进行 Zstandard 压缩。[br][br]
	/// [param p_bytecode] 未压缩的字节码[br]
